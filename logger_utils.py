...
class CustomTrainerCallback(TrainerCallback):
    def __init__(self):
        ...
        self.progress = Progress(SpinnerColumn(), *Progress.get_default_columns(), TimeElapsedColumn(), console=console)
        self.task = None
    ...
    def on_train_begin(self, args, state, control, model=None, train_dataloader=None, **kwargs):
        ...
        console.print(Panel.fit("[bold green]开始训练[/bold green]", title="Training Start"))
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        if state.is_local_process_zero:
            ...
            console.print(Panel.fit(f"[bold green]训练完成![/bold green]\n" f"总耗时: {total_time:.2f}秒\n" f"总步数: {state.global_step}\n" f"最终loss: {final_loss}\n" f"保存路径: {args.output_dir}", title="Training Complete"))
