...
def run_inference(model, tokenizer, prompt):
    ...
    try:
        ...
        # 生成回复，添加控制参数
        with torch.no_grad():
            generated_ids = model.generate(model_inputs.input_ids, attention_mask=model_inputs.attention_mask, max_new_tokens=256, repetition_penalty=1.15, pad_token_id=tokenizer.eos_token_id)  # 明确传递 attention_mask  # 添加重复惩罚 (可以调整 1.1 - 1.3 试试)  # 明确设置 pad_token_id
...
def main(base_model_name, adapter_path):
    ...
    try:
        # 1. 加载量化的基础模型
        base_model = AutoModelForCausalLM.from_pretrained(base_model_name, quantization_config=quantization_config, trust_remote_code=True, device_map="auto")
    ...
    # --- 非交互式测试 ---
    test_prompts = ["EchoHeart是什么？", "你的开发者是谁？", "宋怡敏和罗雨晨有何关系？", "你会做什么？", "你支持多少种语言？"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_model_name", type=str, required=True, help="基础模型的名称或路径 (例如 Qwen/Qwen2.5-1.5B-Instruct)")
    parser.add_argument("--adapter_path", type=str, required=True, help="保存 LoRA 适配器的路径 (例如 output/qwen2-1.5b-qlora-ft)")
