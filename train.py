...
def prepare_dataset(dataset_path, model_name):
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples["messages"])):  # Iterate over samples in the batch
            ...
            if isinstance(current_sample_messages, list):
                try:
                    ...
                    formatted = tokenizer.apply_chat_template(current_sample_messages, tokenize=False, add_generation_prompt=False)  # Important for training!
        ...
        tokenized = tokenizer(formatted_conversations, truncation=True, max_length=512, padding="max_length", return_tensors="pt")  # Pad to max_length
    ...
    tokenized_dataset = dataset.map(preprocess_function, batched=True, remove_columns=dataset["train"].column_names)  # Process in batches  # Remove original columns
...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(args.base_model_name, quantization_config=quantization_config, trust_remote_code=True, device_map="auto")  # <--- 使用命令行参数  # 应用量化配置  # 自动映射设备
    ...
    # 运行测试找到最大 batch size
    determined_batch_size = find_max_batch_size(model, tokenizer, dataset["train"], initial_batch_size=initial_test_bs)  # 使用训练集的一小部分
...
if __name__ == "__main__":
    ...
    # 模型和数据路径
    parser.add_argument("--base_model_name", type=str, required=True, help="要微调的基础 Hugging Face 模型名称或路径")
    parser.add_argument("--dataset_file", type=str, required=True, help="包含 'messages' 字段的 JSON 数据集文件路径")
    parser.add_argument("--output_dir", type=str, default=None, help="训练输出（适配器）保存目录。如果未指定，将基于模型名称自动生成。")
